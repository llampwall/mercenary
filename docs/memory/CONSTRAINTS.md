...
- `sanitizeEnv()` strips all model/endpoint routing vars from the child env — `ANTHROPIC_BASE_URL`, `ANTHROPIC_SMALL_FAST_MODEL`, `CLAUDE_CODE_SUBAGENT_MODEL`, `API_TIMEOUT_MS`, `ALLMIND_LOCAL_MODEL`, `CLAUDE_CODE_REMOTE`, and any `ANTHROPIC_DEFAULT_*` key (case-insensitive prefix) — mirroring allmind `lib/claude-env.js` MODEL_ROUTING_VARS; strips run BEFORE the local-model profile and the `opts.env` merge, so only a deliberate opt-in re-sets routing (added 2026-08-02)
...
- `data/claude-local-model-settings.json`: defensive Bash-deny + force PowerShell tool config for local-model spawns; also sets `"autoCompactWindow": 125000` so a local session compacts before the 131k rig context overflows (updated 2026-08-17)
...
- A leaked `ANTHROPIC_BASE_URL` in the parent env (AllMind local-model lanes set it) silently routes spawned sessions to the local endpoint with no error — `sanitizeEnv()` now strips it, but that strip covers only the headless/one-shot claude path; `sanitizeEnvCodex` and `buildLauncherEnvLines` (interactive launcher) do NOT null routing vars, so an interactive session still inherits a leaked value from the parent process env (added 2026-08-02)
